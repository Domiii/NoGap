    ...
    Client: ComponentDef.defClient(function(Tools, Instance, Context) {
        ...
        return {
            ...
            prepareRequestMetadata: function(metadata) {
                ...
                metadata['x-nogap-v'] = this._indentity.v;
            },
        ...
        return {
            ...
            _registerRPCRequestInitializer: function(router, cfg) {
                ...
                this._registerPostHandler(router, cfg, function(req, res, next) {
                    ...
                    if (!Shared.Libs.ComponentInstance.isCurrentVersion(requestMetadata)) {
                        // wrong version -> refresh client
                    ...
                    var sessionId = req.sessionID;
                    var Instance = req.Instance = Shared.Libs.ComponentInstance.getInstanceMap(sessionId);

                    var promise;
                    if (!Instance) {
                        // create new Instance
                        Instance = req.Instance = this._getOrCreateInstanceForSession(req);

                        // verify Client integrity
                        promise = Instance.Libs.ComponentCommunications.verifyRPCRequestMetadata(requestMetadata)

                        // reactivate
                        .then(Instance.Libs.ComponentBootstrap.reactivateClientInstance.bind(Instance.Libs.ComponentBootstrap));
                    }
                    else {
                        // verify Client integrity
                        promise = Instance.Libs.ComponentCommunications.verifyRPCRequestMetadata(requestMetadata);
