    ...
    Host: ComponentDef.defHost(function(SharedTools, Shared, SharedContext) {
        ...
        return {
            ...
            Private: {
                 ...
                 */
                handleClientRPCRequest: function(clientRequest) {
    ...
    Host: ComponentDef.defHost(function(SharedTools, Shared, SharedContext) {
        ...
        return {
            ...
            setupCommunications: function(app, cfg) {
                ...
                app.use(function patchConnectionState(req, res, next) {
                    ...
                    var Instance = req.Instance = 
                        req.Instance || this._getOrCreateInstanceForSession(req);

                    // TODO: Figure out client state
                    var doesClientExist = ??;

                    if (doesClientExist) {
                        // Client is already bootstrapped but Host Instance is not cached
                        // TODO: Check version
                        if (this.isCurrentVersion(req, req.headers || {})) {
                            // TODO: Fix this
                            var Instance = req.Instance = ;
                            var connection = Instance.Libs.ComponentCommunications.getDefaultConnection();
                            //promise = connection._doBootstrapClientInstance(req, res, next);
                        }
                    }
            ...
            getClientVersion: function(requestMetadata) {
                return req.headers && req.headers['x-nogap-v'];
            },
             ...
             */
            isCurrentVersion: function(connectionState, requestMetadata) {
                return this.getClientVersion(requestMetadata);
            },

            _getOrCreateInstanceForSession: function(req) {
                ...
                var Instance = Shared.Libs.ComponentInstance.activateSession(
                    session, sessionId, doesClientExist);
            ...
            _registerBootstrapRequestHandler: function(router, cfg) {
                router.get(cfg.baseUrl + "*", function(req, res, next) {
                    ...
                    return this._doBootstrapClientInstance(req, res, next)
                    .bind(this)

                    // send bootstrap code to Client and kick things of there
                    .then(function(codeString) {
                        this._sendBootstrapRequestToClient(res, codeString);
                    })
            ...
            _registerRPCRequestHandler: function(router, cfg) {
                router.post(cfg.baseUrl, function(req, res, next) {
                    ...
                    return this.Instance.Libs.ComponentCommunications.
                        verifyRequestMetadata(requestMetadata)
                    .then(function() {
                        // get Instance
                        var Instance = req.getInstance();
                        ...
                        req.on('end', function () {
                            ...
                            Promise.resolve()
                            .then(function() {
                                return connection.handleClientRPCRequestString(body, requestMetadata, res);
                            })
                            .finally(function() {
                                connection._currentConnectionState = null;
                            });
            ...
            Private: {
                ...
                handleClientRPCRequest: function(clientRequest, requestMetadata, connectionState) {
                    ...
                    return this.Instance.Libs.ComponentCommunications.handleClientRPCRequest(clientRequest)
                    .then(function(hostResponseData) {
