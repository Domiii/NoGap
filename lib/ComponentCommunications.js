...
/**
 * A registry of all existing ComponentEndpointImpl types.
 */
    ...
    Host: ComponentDef.defHost(function(SharedTools, Shared, SharedContext) {
        ...
        return {
            ...
            Private: {
                ...
                /**
                 * Flushes the current buffer, if connection is not still kept open.
                 * This is generally only needed for connections that are not always open, 
                 * and need to be kept open for asynchronous transactions.
                 */
                flush: function() {
                    --this.keptOpen;
...
/**
 * Uses express for routing post requests.
 */
var HttpPostImpl = {
    ImplName: "HttpPost",
        ...
        return {
            ...
            initHost: function(app, cfg) {
                squishy.assert(app.post, 'Invalid argument for initHost: `app` does not have a `post` method. ' +
                    'Make sure to pass an express application object to `ComponentLoader.start` when using HttpPost.');
            ...
            Private: {
                ...
                /**
                 * This host-side function is called when a bunch of client commands are to be flushed to the client.
                 */
                    ...
                    xhReq.onreadystatechange = function() {
                        ...
                        if (xhReq.status==200) {
                            if (xhReq.responseText) {
                                // host sent commands back:
                                var commands;
                                try {
                                    //commands = JSON.parse(xhReq.responseText);
                                    commands = eval(xhReq.responseText);
                                }
                                catch (err) {
                                ...
                                // execute the commands that the host sent back, as response to the client request
                                Instance.Libs.CommandProxy.execHostCommands(commands);
                            }
