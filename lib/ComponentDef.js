...
var ComponentDefDef = {
    ...
    Host: HostCore.defHost(function(Tools, Shared) {
        ...
        return {
             ...
             * Also, recursively adds all `Includes` of those components.
            ...
            getClientComponentDefsForDeployment: function(componentNames) {
                ...
                this._getClientComponentDefsForDeployment(componentNames, 0, defs, uniqueNames);

                // we finished recursion -> return array of all requested component definitions
                return defs;
            },

            _getClientComponentDefsForDeployment: function(componentNames, nDepth, defs, uniqueNames) {
                for (var i = 0; i < componentNames.length; ++i) {
                    ...
                    // ignore definition, if it has been added previously
                    // to prevent circular inclusion
                    if (!uniqueNames[componentName]) {
                        defs.push(def);
                        uniqueNames[def.FullName] = true;

                        if (def.Includes) {
                            if (nDepth > 16) {
                                // something PROBABLY went wrong here
                                throw new Error('Possible bug detected: Dangerously deep inclusion - ' +
                                    'Inclusion depth > 16 at component `' + componentName + '`.');
                            }

                            // recursively add all includes
