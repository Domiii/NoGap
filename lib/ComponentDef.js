    ...
    Base: HostCore.defBase(function(Tools, Shared, Context) {
        ...
        return {
            ...
            finishInstalledComponentEndpoint: function(componentEndpoint, base, sharedMap) {
                ...
                // check integrity
                console.assert(!componentEndpoint.host, 'Component `' + componentEndpoint + '` defined reserved property `host`. Remove it!');
                console.assert(!componentEndpoint.client, 'Component `' + componentEndpoint + '` defined reserved property `client`. Remove it!');
                console.assert(!componentEndpoint.clients, 'Component `' + componentEndpoint + '` defined reserved property `clients`. Remove it!');
            ...
            installComponent: function(sharedMap, componentDef, endpointName) {
                ...
                try {
                    ...
                    componentEndpoint = this.mergeBaseIntoComponentEndpoint(endpointDef, endpointName, base, componentEndpoint);
            ...
            mergeBaseIntoComponentEndpoint: function(componentDef, nameTo, base, componentEndpoint) {
                ...
                var componentEndpoint = this.doMerge('Component `' + componentDef.FullName + '`', 'Base', nameTo, base, componentEndpoint);
             ...
             */
            doMerge: function(ownerName, nameFrom, nameTo, from, to) {
                if (from && to) {
                    // then, deep-merge the rest
                    for (var propName in from) {
                        if (!from.hasOwnProperty(propName)) continue;
                        if (from[propName] && to[propName]) {
                            if (typeof(from) === 'object' && typeof(to) === 'object') {
                                // nested object -> Merge recursively
                                to[propName] = this.doMerge(ownerName, nameFrom + '.' + propName, nameTo + '.' + propName, from[propName], to[propName]);
                            }
                            else {
    ...
    Host: HostCore.defHost(function(Tools, Shared) {
        ...
        var fixComponentDefinition = function(componentDef, endpointName) {
            ...
            if (!endpointDef) {
                ...
                componentDef[endpointName] = endpointDef = HostCore[defFunName](function(Components) { return {}; });
            }
        ...
        var installComponentOnHost = function(componentDef, clientDefs, map, creationFrame) {
            ...
            // add client def
            var clientDef = {
                Client: componentDef.Client,
                Base: componentDef.Base
            };
        ...
        /**
         * This function is the first thing to the run on the client to install the components framework.
         */
        ...
        return {
             ...
             */
            getClientComponentDefs: function(componentNames) {
                var defs = [];
                for (var i = 0; i < componentNames.length; ++i) {
                    var compName = componentNames[i];
                    var def = clientDefs.Components.map[compName];
                    console.assert(def, 'Invalid component selected - It does not exist or does not have a `Client` definition: ' + compName);
