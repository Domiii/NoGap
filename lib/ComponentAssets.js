...
module.exports = ComponentDef.lib({
    Base: ComponentDef.defBase(function(SharedTools, Shared, SharedContext) {
        return {
            getPublicPath: function(componentName, fileName) {
                var path = this.publicPath + (componentName ? (componentName + '/') : '');
                if (fileName) {
                    path += fileName;
                }
                return path;
    ...
    Host: ComponentDef.defHost(function(SharedTools, Shared) {
            ...
            req.addListener('end', function () {
                ...
                fs.readFile(fpath, function (err, data) {
                    ...
                    if (err) {
                        ...
                        //console.error('Sending file: ' + fpath);
        ...
        return {
            ...
            initHost: function(app, cfg) {
                // setup paths & URLs
                this.publicPath = cfg.publicPath;
                try {
                    pubFolderAbs = fs.realpathSync(this.publicPath);
                }
                catch (err) {
                    throw new Error('Public folder does not exist: ' + this.publicPath);
                }

                if (!this.publicPath.startsWith('/')) {
            ...
            getPublicUrl: function(cfg) {
                var publicUrl = url.resolve(baseUrl, this.publicPath);
            ...
            Private: {
                getClientCtorArguments: function() {
                    return [publicUrl, this.Shared.publicPath];
                },
            }
        };
    }),
    ...
    Client: ComponentDef.defClient(function(Tools, Instance, Context) {
        return {
            ...
            __ctor: function(publicUrl, publicPath) {
                this.publicUrl = publicUrl;
                this.publicPath = publicPath;
                console.assert(publicUrl && publicPath, 
                    'ComponentAsset.Client has not initialized correctly');
            },
